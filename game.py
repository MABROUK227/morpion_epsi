...
# Actions possibles disponibles sur le tableau
def actions(board):
    moves_available = set()
    for i in range(3):
        for j in range(3):
...
# Résultat après un mouvement (i & j)
...
#  Retourne 1 si X a gagné le jeu, -1 si O a gagné, 0 sinon
def utility(board):
    win = winner(board)
    if win == X:
        return 1
...
# Retourne la valeur d'utilité minimale sur tous les mouvements légaux
def min_value(board):
    if terminal(board):
...
# Retourne la valeur d'utilité maximale sur tous les mouvements légaux
def max_value(board):
    if terminal(board):
        return utility(board), None
...
# Retourne l'action optimale pour le joueur actuel
def minimax(board):
    if terminal(board):
        return None
    if player(board) == X:
